...
class ImageProcess():
    ...
    def image_process(self, img_path):
        ...
        x_grid = cv2.Sobel(img_gray, cv2.CV_16SC1, 1, 0)
        y_grid = cv2.Sobel(img_gray, cv2.CV_16SC1, 0, 1)
        img_canny = cv2.Canny(x_grid, y_grid, 30, 200)
        ...
        for i in range(len(contours) // 2):
            if i % 2 == 0:# Even
                ...
                rect_likelihood = cv2.matchShapes(contours[i],self.contours_rect, 1, 0.0)
                elip_likelihood = cv2.matchShapes(contours[i],self.contours_elip, 1, 0.0)
                if rect_likelihood < elip_likelihood:
                    shape.append(0)
                else:
                    shape.append(1)

                N = cv2.moments(contours[i * 2]) # TODO
                x = int(N["m10"] / N["m00"])
                y = int(N["m01"] / N["m00"])
                ...
                x, y = center_value[i-1]
                theta_tmp = np.arctan(_center_y - y, _center_x - x)
